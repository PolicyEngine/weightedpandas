...
class WeightedSeries(pd.Series):
    ...
    def mean(self, *args, **kwargs) -> float:
        ...
        if weights_sum == 0:
            return np.nan
        return pd.Series(self.values * self._weights.values).sum(*args, **kwargs) / weights_sum
    ...
    def var(self, ddof: int = 1, *args, **kwargs) -> float:
        ...
        return pd.Series(((self.values - wmean) ** 2) * self._weights.values).sum(*args, **kwargs) / (weights_sum - ddof)
...
class WeightedDataFrame(pd.DataFrame):
    ...
    def var(self, axis=0, ddof=1, *args, **kwargs):
        ...
        if axis == 0:
            ...
            for col in self.columns:
                values = self[col].values
                weights = self._weights.values
                result[col] = np.sum(((values - wmeans[col]) ** 2) * weights, *args, **kwargs) / (weights_sum - ddof)
            return pd.Series(result)
    ...
    def corr(self, method="pearson", min_periods=1):
        ...
        if method != "pearson":
            raise ValueError("Only Pearson correlation is implemented for weighted DataFrames")
        ...
        for i in self.columns:
            ...
            wstd[i] = np.sqrt(np.sum(((self[i].values - wmeans[i]) ** 2) * self._weights.values) / self._weights.sum())
            
            for j in self.columns:
                # Covariance between columns i and j
                wcov.loc[i, j] = np.sum(
                    (self[i].values - wmeans[i]) * (self[j].values - wmeans[j]) * self._weights.values
                ) / self._weights.sum()
