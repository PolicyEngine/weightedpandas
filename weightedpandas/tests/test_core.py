...
class TestWeightedSeries(unittest.TestCase):
    ...
    def test_mean(self):
        expected = sum(d * w for d, w in zip(self.data, self.weights)) / sum(self.weights)
        self.assertAlmostEqual(self.ws.mean(), expected)
    
    def test_var(self):
        wmean = self.ws.mean()
        expected = sum(((d - wmean) ** 2) * w for d, w in zip(self.data, self.weights)) / (sum(self.weights) - 1)
...
class TestWeightedDataFrame(unittest.TestCase):
    ...
    def test_mean(self):
        expected_a = sum(d * w for d, w in zip(self.data['A'], self.weights)) / sum(self.weights)
        expected_b = sum(d * w for d, w in zip(self.data['B'], self.weights)) / sum(self.weights)
    ...
    def test_var(self):
        ...
        expected_a = sum(((d - wm_a) ** 2) * w for d, w in zip(self.data['A'], self.weights)) / (sum(self.weights) - 1)
        
        wm_b = self.wdf['B'].mean()
        expected_b = sum(((d - wm_b) ** 2) * w for d, w in zip(self.data['B'], self.weights)) / (sum(self.weights) - 1)
